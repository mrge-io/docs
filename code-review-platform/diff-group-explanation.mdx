...
AI change visualizations work **hand-in-hand with intelligent file ordering**. When cubic clusters related files into a logical change-set, it decides—on a per-group basis—whether an AI summary and visual diff will meaningfully speed up the review.

If the edits are straightforward, the group is shown without extra visuals to keep the page light; if the changes are dense or spread across several files, cubic adds the visualization automatically.
...
- **Visual diff** _(optional)_: A side-by-side snippet or small diagram that highlights the most significant before/after. The diagram only appears if it adds clarity; simpler groups may show the summary alone.
