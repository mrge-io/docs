...
<Note>
  **Prerequisites** 
  - [Create a stack](/docs/how-to-guides/create-a-stack) 
...
<Steps>
  ...
  <Step title="Make changes and commit">
    ```bash 
    mg add . 
    mg commit -m "Address feedback" # or mg modify to amend 
    ```
  </Step>
...
<Steps>
  <Step title="Split commits">
    Breaks large commits into smaller ones:
    ```bash
    mg split
    ```
  </Step>
  <Step title="Fold/squash commits">
    If you want to combine commits:
    ```bash
    mg split --fold
    ```
  </Step>
...
<Tip>
  - **Ready to merge?** See [Merge your stack](/docs/how-to-guides/merge-your-stack).
  - **Troubleshooting?** Check our [FAQ](/docs/faq-and-troubleshooting).
</Tip>
